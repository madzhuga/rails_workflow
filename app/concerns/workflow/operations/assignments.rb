  ...
  module Operations
    module Assignments
      ...
      included do
        ...
        scope :by_role, -> (role) { joins(:template).where(workflow_operation_templates: { role: role })}
        scope :by_group, -> (group) { joins(:template).where(workflow_operation_templates: { group: group })}
